...
export class SonarQubeClient {
  ...
  async getQualityMetrics(projectKey: string): Promise<QualityMetrics> {
    ...
    return {
      ...
      coverage: parseFloat(measureValue(m, 'coverage')),
      duplicatedLinesDensity: parseFloat(measureValue(m, 'duplicated_lines_density')),
      linesOfCode: parseInt(measureValue(m, 'ncloc'), 10),
      technicalDebt: minutesToIso8601(parseInt(measureValue(m, 'sqale_index'), 10)),
      issues: {
        blocker: parseInt(measureValue(m, 'blocker_violations'), 10),
        critical: parseInt(measureValue(m, 'critical_violations'), 10),
        major: parseInt(measureValue(m, 'major_violations'), 10),
        minor: parseInt(measureValue(m, 'minor_violations'), 10),
        info: parseInt(measureValue(m, 'info_violations'), 10),
      },
